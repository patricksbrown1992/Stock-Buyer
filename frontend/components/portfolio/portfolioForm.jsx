...
class PortfolioForm extends React.Component {
      ...
      .then(() =>
        Object.values(this.props.businesses).forEach((business) => {
          getPrice(business.ticker)
            ...
            .then((res) =>
              this.props.updateBusiness(this.props.user.id, {
                price_now: res.latestPrice,
                id: business.id,
                user_id: this.props.user.id,
                purchase_price: business.purchase_price,
                net_shares: business.net_shares,
  ...
  handleCheck() {
    ...
    if (!Number.isInteger(quant) || this.state.qty.includes(".") || quant < 1) {
      ...
      return getPrice(ticker).then(
        (ele) =>
          this.setState({
            cost: (ele.latestPrice * quant).toFixed(2),
            price: ele.latestPrice,
          }),
        (err) => this.props.portfolioTicker()
      );
    }
  }
  ...
  handleClickBuy(e) {
    ...
    if (this.props.businesses[ticker]) {
      ...
      this.handleCheck(ticker)
        ...
        .then(() =>
          this.props.createBusiness(user.id, {
            user_id: user.id,
            ticker: ticker,
            net_shares: quant,
            purchase_price: this.state.price,
  ...
  render() {
    ...
    if (Object.values(this.props.businesses).length < 1) {
      ...
      businesses = Object.values(this.props.businesses).map((business) => {
        ...
        let percent = (
          (business.price_now / business.purchase_price) * 100 -
          100
        ).toFixed(2);
        total += count;
        ...
        if (business.price_now > business.purchase_price) {
          return (
            <li id="green" className="transaction-li" key={business.id}>
              {business.ticker} - {business.net_shares} shares $
              {count.toFixed(2)} ({percent}%)
            </li>
          );
      ...
      <div className="portfolio-page">
        ...
        <div className="portfolio-body">
          <div className="portfolio-left">
            ...
            <br />
            <p id="transactions-title">
              Current Portfolio Value ${total.toFixed(2)}
            </p>
